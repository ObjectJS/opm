...
function install(name) {
	...
	registry.get(folder, function(err, data, json, response) {
		...
		request(tarUrl).pipe(fs.createWriteStream(tarPath)).on('close', function() {
			fs.createReadStream(tarPath).pipe(zlib.Unzip()).pipe(tar.Extract({path: cachePath})).on('end', function() {
				...
				var targetPath = path.join(process.cwd(), 'node_modules/' + name);
				mkdirp(targetPath);
...
module.exports.usage = 'install <name>';
