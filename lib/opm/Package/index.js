...
Package.packageGetters = [
	function(packageRoot, callback) {
		fs.exists(path.join(packageRoot, 'components'), function(exists) {
			callback(null, exists? new (require('../ProjectPackage'))(packageRoot) : null);
		});
	},
	function(packageRoot, callback) {
		fs.exists(path.join(packageRoot, '../../components'), function(exists) {
			callback(null, exists? new (require('../ComponentPackage'))(packageRoot) : null);
		});
	},
	function(packageRoot, callback) {
		callback(null, new (require('../Package'))(packageRoot));
	}
];

Package.getPackage = function(packageRoot, callback) {
	var found;
	async.detectSeries(Package.packageGetters, function(getter, callback) {
		getter(packageRoot, function(err, package) {
			found = package;
			callback(!!package);
		});
	}, function() {
